                        ...
                        <tbody>
                            ...
                            <?php foreach ($roles as $role) : ?>
                                <tr>
                                    ...
                                    <td><?= esc($role['url']) ?></td>
                                    <td>
                                        <a href="<?= site_url('superadmin/navigation/' . $role['id']) ?>" class="btn btn-success btn-sm">
                                            <i class="bi bi-card-list"></i> Data Menu
                                        </a>
                                        <a data-toggle="modal" data-target="#editRowModal" data-role_id="<?= esc($role['id']) ?>" class="btn btn-warning btn-sm">
                                            <i class="bi bi-pencil-square"></i> Edit
                                        </a>
                                        <a href="<?= site_url('superadmin/role/delete/' . $role['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus role ini?')">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('superadmin/role/add') ?>" method="POST">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="form-group form-group-default">
                                <label>URL</label>
                                <input id="addPosition" type="text" class="form-control" placeholder="Url <?= $title ?>" name="url" required style="padding-left: 10px;">
                            </div>
                        </div>
                    </div>
            </div>
...
<div class="modal fade" id="editRowModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('superadmin/role/update/') ?>" method="POST">
...
<script>
    $(function() {
        ...
        $('#editRowModal').on('show.bs.modal', function(e) {
            ...
            $.ajax({
                type: 'post',
                url: '<?= base_url('Ajax/edit_role/') ?>' + id,
                data: {
                    'id': id,
